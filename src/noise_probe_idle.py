...
state_preps = {
    ...
    "1":  lambda qc: qc.x(0),
    "+":  lambda qc: qc.h(0),
    "-":  lambda qc: (qc.x(0), qc.h(0)),   # H|1⟩ = |−⟩
    "+i": lambda qc: (qc.h(0), qc.s(0)),   # S H|0⟩ = |+i⟩
    "-i": lambda qc: (qc.h(0), qc.sdg(0)), # S† H|0⟩ = |−i⟩
}
...
def analyse_bloch(
    ...
    for i in range(num_points):
        ...
        for j, basis in enumerate(("X", "Y", "Z")):
            counts = results[i * 3 + j].data.meas.get_counts()
            p0 = counts.get("0", 0)
            p1 = counts.get("1", 0)
    ...
    # ------------------------------------------------------------------
        ...
        if args.mode == "population":
            circuits = build_population_circuits(
                args.num_points, args.gates_per_point, pm, args.initial_state
            )
