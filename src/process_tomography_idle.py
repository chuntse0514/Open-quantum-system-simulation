...
def make_paths(
    ...
    folder = Path("results/process_tomography/idle") / backend 
...
def run_process_tomography(
    backend, sampler, num_points: int, gates_per_point: int, shots: int, qubit: int = 0
):  
    idle_circuits = build_idle_circuits(num_points, gates_per_point)
    choi_matrices = []
    channel_fidelities = []
...
def main():
    ...
    parser.add_argument("-q",    "--qubit",          type=int, default=0, help="Physical qubit index")
    ...
    choi_matrices, channel_fidelities = run_process_tomography(backend, sampler, args.num_points, args.gates_per_point, args.shots, args.qubit)
